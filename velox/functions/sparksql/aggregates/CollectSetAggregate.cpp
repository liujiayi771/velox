...
#include "velox/functions/sparksql/aggregates/CollectSetAggregate.h"
...
public:
 ...
 struct AccumulatorType {
   SetAccumulator<Generic<T1>> elements_;

   AccumulatorType() = delete;

   explicit AccumulatorType(HashStringAllocator* allocator) {
     elements_ = SetAccumulator<Generic<T1>>{BIGINT(), allocator};
   }
...
AggregateRegistrationResult registerCollectSet(
 ...
 return exec::registerAggregateFunction(
     ...
     [name](
       ...
       return std::make_unique<SimpleAggregateAdapter<CollectSetAggregate>>(
           resultType);
     },
...
void registerCollectSetAggregate(
   ...
   bool overwrite) {
 registerCollectSet(
     prefix + "collect_set", withCompanionFunctions, overwrite);
}
} // namespace facebook::velox::functions::aggregate::sparksql