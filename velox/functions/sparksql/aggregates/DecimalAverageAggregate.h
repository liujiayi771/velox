...
class DecimalAverageAggregate {
 public:
  ...
  static void initialize(
      DecimalAverageAggregate<TInputType, TResultType>* fn,
      core::AggregationNode::Step step,
      const std::vector<TypePtr>& argTypes,
      const TypePtr& resultType) {
    VELOX_CHECK_LE(argTypes.size(), 1);
    auto inputType = argTypes[0];
    fn->sumType = getSumType(inputType);
  ...
  struct AccumulatorType {
    ...
    DecimalAverageAggregate<TInputType, TResultType>* fn_;
    ...
    std::optional<int128_t> computeFinalResult() const {
      ...
      auto [resultPrecision, resultScale] =
          getDecimalPrecisionScale(*fn_->resultType.get());
      auto [sumPrecision, sumScale] =
          getDecimalPrecisionScale(*fn_->sumType.get());
