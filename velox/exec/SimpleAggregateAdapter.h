...
class SimpleAggregateAdapter : public Aggregate {
 public:
  ...
  // SimpleAggregateAdapter::accumulatorAlignmentSize() returns alignof(typename
  // FUNC::AccumulatorType).
  ...
  template <typename T>
  struct aligned_accumulator<
      T,
      std::void_t<decltype(T::aligned_accumulator_)>>
  ...
  static constexpr bool aligned_accumulator_ =
      aligned_accumulator<typename FUNC::AccumulatorType>::value;
